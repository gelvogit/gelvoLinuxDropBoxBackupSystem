...
define('DROPBOX_KEY',			'dropbox_key');
define('DROPBOX_SECRET',		'dropbox_secret');
define('DROPBOX_TOKEN',			'dropbox_token');
define('DROPBOX_CHUNKSIZE',		'dropbox_chunksize');
...
function build_backup_session($options) {
	...
	$backupSessionObj->config[MONTH]                = date(MONTH_DATE_FORMAT);
...
function _populate_config_array($xmlOBJ) {
	...
	$result[DROPBOX_CHUNKSIZE]	= (int)$xmlOBJ->config->dropbox_chunksize;	
